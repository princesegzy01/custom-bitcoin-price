...
describe("GraphQL Resolver Endpoint", function () {
    ...
    it("Should return a Markup Price", function (done) {
        request
            .post("/graphiql")
            .send({
            query: ' { calculatePrice(type:"buy", margin : 0.2, exchangeRate :300) }'
        })
            .expect(200)
    ...
    it("Should retun Margin cannot be greater than 1", function (done) {
        request
            .post("/graphiql")
            .send({
            query: ' { calculatePrice(type:"buy", margin : 5, exchangeRate :300) }'
        })
            .expect(200)
            .end(function (err, res) {
            if (err) {
    ...
    it("Should return Margin cannot be lesser or equal to 0", function (done) {
        request
            .post("/graphiql")
            .send({
            query: ' { calculatePrice(type:"buy", margin : 0, exchangeRate :300) }'
        })
            .expect(200)
            .end(function (err, res) {
            if (err) {
    ...
    it("Should return Invalid action type", function (done) {
        request
            .post("/graphiql")
            .send({
            query: ' { calculatePrice(type:"buys", margin : 0.3, exchangeRate :300) }'
        })
            .expect(200)
            .end(function (err, res) {
            if (err) {
    ...
    it("Should return Exchange rate cannot be lesser or equal to 0", function (done) {
        request
            .post("/graphiql")
            .send({
            query: ' { calculatePrice(type:"buy", margin : 0.3, exchangeRate :-240) }'
        })
            .expect(200)
            .end(function (err, res) {
            if (err) {
    ...
    it("Should return 400 Bad request", function (done) {
        request
            .post("/graphiql")
            .send({
            query: ' { calculatePrice(type:"buy", margin : 0.3, exchangeRate : 240.8) }',
        })
            .expect(400)
            .end(function (err, res) {
            if (err) {
