...
const root = {
	...
	calculatePrice: async ({ type, margin, exchangeRate }: priceArgs) => {
		// set margin price
		let marginPrice = 0.0;
		...
		if (margin <= 0) {
			return "Margin cannot be lesser or equal to 0";
		}
		...
		if (exchangeRate <= 0) {
			return "Exchange rate cannot be lesser or equal to 0";
		}
		...
		try {
			...
			return "Unable to fetch bitcoin price from coindesk API";
		}
		...
		const ngnPrice: number = exchangeRate * totalPrice;
